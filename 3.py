...
class Cell:
    ...
    def __sub__(self, other):
        if self.nucleus - other.nucleus > 0:
            ...
            return "Разность количества ячеек меньше 0."
    ...
    def make_order(self, n_in_line):
        out = ""
        for i in range(self.nucleus // n_in_line):
            for star in range(n_in_line):
                out += "*"
            out += "\n"
        tail = self.nucleus % n_in_line
        if tail > 0:
            for star in range(tail):
                out += "*"
        return out
